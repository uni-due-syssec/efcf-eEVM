...
namespace eevm
{
  const uint64_t CALL_GAS_STIPEND = 2300;
  ...
  enum Opcode : uint8_t
  {
    ...
    SHL = 0x1b, // logical shift left operation
    SHR = 0x1c, // logical shift right operation
    SAR = 0x1d, // arithmetic shift right operation
    ...
    SELFBALANCE = 0x47, // get balance of own account
    ...
    CREATE2 = 0xf5, // Create a new account with associated code. Exactly
                    // equivalent to CREATE except: The salt ζ ≡ μs[3].
    STATICCALL =
      0xfa, // Static message-call into an account. Exactly equivalent to CALL
            // except: The argument µs is replaced with 0.
